...
static void *pthread_hash_numeric(void *param) {
	...
	csr_t __attribute__((unused)) *csr_c = arg->csr_c;
	bin_t *bin = arg->bin;

	my_cnt_t start_row = bin->rows_offset[_tid];
	my_cnt_t end_row = bin->rows_offset[_tid+1];

	my_cnt_t *ht_check = bin->local_hash_table_id[_tid];
	my_rat_t *ht_value = bin->local_hash_table_val[_tid];
	...
	for (my_cnt_t idx = start_row; idx < end_row; idx++) {
		...
		if (bid > 0) {
			...
			my_cnt_t ht_size = MIN_HT_S << (bid - 1);

			memset(ht_check, -1, sizeof(my_cnt_t)*ht_size);
			...
			for (my_cnt_t jdx = csr_a->row_ptrs[idx]; jdx < csr_a->row_ptrs[idx+1]; jdx++) {
				...
				my_cnt_t t_aval = csr_a->rats[jdx];

#ifdef IN_EXAMINE
				_flop_cnt += csr_b->row_ptrs[t_acol+1] - csr_b->row_ptrs[t_acol];
#endif /* IN_EXAMINE */

				for (my_cnt_t kdx = csr_b->row_ptrs[t_acol]; kdx < csr_b->row_ptrs[t_acol+1]; kdx++) {
					my_rat_t t_val = func_op(t_aval, csr_b->rats[kdx]);
			...
			for (my_cnt_t jdx = 0; jdx < ht_size; jdx++) {
				if (ht_check[jdx] != -1) {
					csr_c->cols[offset+index] = ht_check[jdx];
					csr_c->rats[offset+index] = ht_value[jdx];
					index++;
				}
			}
...
static void hash_numeric(csr_t* csr_a, csr_t* csr_b, csr_t* csr_c,
	bin_t* bin, my_rat_t (*func_op)(my_rat_t, my_rat_t)) {
	...
	hash_numeric_arg_t args[THREAD_NUM];
	memset(args, 0, sizeof(hash_numeric_arg_t)*THREAD_NUM);
	...
	for (int idx = 0; idx < THREAD_NUM; idx ++) {
		...
		rv = pthread_create(&threadpool[idx], &attr, pthread_hash_numeric, args+idx);
		if (rv){
	...
	for (int idx = 0; idx < THREAD_NUM; idx ++) {
		...
		printf("\t_set_time: %.9f\t_comp_time: %.9f\t_write_time: %.9f", 
			args[idx]._set_time, args[idx]._comp_time, args[idx]._write_time
		);
		printf("\t_row_cnt: %ld\t_flop_cnt: %ld\t_nnzc_cnt: %ld",
			bin->rows_offset[idx+1]-bin->rows_offset[idx], args[idx]._flop_cnt,
			csr_c->row_ptrs[bin->rows_offset[idx+1]] - csr_c->row_ptrs[bin->rows_offset[idx]]
		);
		printf("\t_cfi: %.6f\t_cfo: %.6f", 
			(double) args[idx]._flop_cnt/(csr_a->row_ptrs[bin->rows_offset[idx+1]] - csr_a->row_ptrs[bin->rows_offset[idx]]),
			(double) args[idx]._flop_cnt/(csr_c->row_ptrs[bin->rows_offset[idx+1]] - csr_c->row_ptrs[bin->rows_offset[idx]])
		);
		printf("\t_row_range: %ld-%ld", 
			bin->rows_offset[idx], bin->rows_offset[idx+1]
		);
#endif /* IN_EXAMINE */
		printf("\n");
		if (idx == THREAD_NUM-1) {
...
static runtime_t hashspgemm(csr_t* csr_a, csr_t* csr_b, csr_t* csr_c, numa_cfg_t* numa_cfg) {
	...
	bin_t bin;
	init_bin(&bin, csr_a->num_row, MIN_HT_S, numa_cfg);

	// 1st pass, csr_a->row_ptrs, csr_a->cols, csr_b->row_ptrs
	set_intprod_num(&bin, csr_a, csr_b);
	hash_set_rows_offset(&bin, csr_c->num_row, numa_cfg);
	set_bin_id(&bin, csr_c->num_row, csr_c->num_col);
	bin_create_local_hash_table(&bin, csr_c->num_col, numa_cfg);
	...
	// 2nd pass, csr_a->row_ptrs, csr_a->cols, csr_b->row_ptrs, csr_b->cols
	...
	hash_symbolic(csr_a, &intr_csr_b, &bin);
	...
	hash_symbolic(csr_a, csr_b, &bin);
#endif /* defined(MEMSV) && defined(MEMSVB) */ 

	scan(csr_c->row_ptrs, bin.row_nz, csr_c->num_row+1);
	...
	hash_numeric(csr_a, csr_b, csr_c, &bin, func_mul);
