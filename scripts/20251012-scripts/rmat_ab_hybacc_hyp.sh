...
DIR_PATH=${CUR_DIR}/../../logs/20251012-logs
mkdir -p $DIR_PATH
...
AB_ACC_THRES=2.00
ER_1_AB_HYBACC_HYP_LOG=${DIR_PATH}/er_1_ab_hybacc_hyp.log
rm $ER_1_AB_HYBACC_HYP_LOG
G500_1_AB_HYBACC_HYP_LOG=${DIR_PATH}/g500_1_ab_hybacc_hyp.log
rm $G500_1_AB_HYBACC_HYP_LOG
SSCA_1_AB_HYBACC_HYP_LOG=${DIR_PATH}/ssca_1_ab_hybacc_hyp.log
rm $SSCA_1_AB_HYBACC_HYP_LOG
cd ../..
bash clean.sh
mkdir build
cd build
...
for scale in 18 19 20 21; do
	for edge_factor in 1 2 4 8 16 32; do
		...
		$NUMACTL --physcpubind=$CORE_RANGE ./bin/ab_hybaccspgemm $dataset >> $ER_1_AB_HYBACC_HYP_LOG 2>&1
	done
done
for scale in 17 18 19 20; do
	for edge_factor in 1 2 4 8 16 32; do
		dataset="g500_${scale}_${edge_factor}"
		$NUMACTL --physcpubind=$CORE_RANGE ./bin/ab_hybaccspgemm $dataset >> $G500_1_AB_HYBACC_HYP_LOG 2>&1
	done
done
for scale in 17 18 19 20; do
	for edge_factor in 1 2 4 8 16 32; do
		dataset="ssca_${scale}_${edge_factor}"
		$NUMACTL --physcpubind=$CORE_RANGE ./bin/ab_hybaccspgemm $dataset >> $SSCA_1_AB_HYBACC_HYP_LOG 2>&1
	done
done
bash clean.sh
cd ${CUR_DIR}
