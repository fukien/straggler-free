...
TAMU_6_HSERSC_LOG=${DIR_PATH}/tamu_6_hsersc.log
rm $TAMU_6_HSERSC_LOG
...
for dataset in ${tamu_dataset_list[@]}; do
	$NUMACTL --physcpubind=$CORE_RANGE ./bin/hserscspgemm $dataset >> $TAMU_6_HSERSC_LOG 2>&1
done
bash clean.sh
cd ${CUR_DIR}
...
TAMU_6_HSERSC_HYP_LOG=${DIR_PATH}/tamu_6_hsersc_hyp.log
rm $TAMU_6_HSERSC_HYP_LOG
THREAD_NUM=$(numactl --hardware | awk '/node 0 cpus:/ {print (NF-3)/2}')
...
cd ..
for dataset in ${tamu_dataset_list[@]}; do
	$NUMACTL --physcpubind=$CORE_RANGE ./bin/hserscspgemm $dataset >> $TAMU_6_HSERSC_HYP_LOG 2>&1
done
bash clean.sh
cd ${CUR_DIR}
...
TAMU_1_HSERSC_LOG=${DIR_PATH}/tamu_1_hsersc.log
rm $TAMU_1_HSERSC_LOG
THREAD_NUM=$(numactl --hardware | awk '/node 0 cpus:/ {print (NF-3)/2}')
...
cd ..
for dataset in ${tamu_dataset_list[@]}; do
	$NUMACTL --physcpubind=$CORE_RANGE ./bin/hserscspgemm $dataset >> $TAMU_1_HSERSC_LOG 2>&1
done
bash clean.sh
cd ${CUR_DIR}
...
TAMU_1_HSERSC_HYP_LOG=${DIR_PATH}/tamu_1_hsersc_hyp.log
rm $TAMU_1_HSERSC_HYP_LOG
THREAD_NUM=$(numactl --hardware | awk '/node 0 cpus:/ {print (NF-3)/2}')
THREAD_NUM=$((2 * THREAD_NUM))
CORE_RANGE="0-31,64-95"
cd ../..
...
cd ..
for dataset in ${tamu_dataset_list[@]}; do
	$NUMACTL --physcpubind=$CORE_RANGE ./bin/hserscspgemm $dataset >> $TAMU_1_HSERSC_HYP_LOG 2>&1
done
bash clean.sh
cd ${CUR_DIR}
