...
TAMU_6_HEAP_LOG=${DIR_PATH}/tamu_6_heap.log
rm $TAMU_6_HEAP_LOG
...
for dataset in ${tamu_dataset_list[@]}; do
	$NUMACTL --physcpubind=$CORE_RANGE ./bin/heapspgemm $dataset >> $TAMU_6_HEAP_LOG 2>&1
done
bash clean.sh
cd ${CUR_DIR}
...
TAMU_6_HEAP_HYP_LOG=${DIR_PATH}/tamu_6_heap_hyp.log
rm $TAMU_6_HEAP_HYP_LOG
THREAD_NUM=$(numactl --hardware | awk '/node 0 cpus:/ {print (NF-3)/2}')
...
cd ..
for dataset in ${tamu_dataset_list[@]}; do
	$NUMACTL --physcpubind=$CORE_RANGE ./bin/heapspgemm $dataset >> $TAMU_6_HEAP_HYP_LOG 2>&1
done
bash clean.sh
cd ${CUR_DIR}
...
TAMU_1_HEAP_LOG=${DIR_PATH}/tamu_1_heap.log
rm $TAMU_1_HEAP_LOG
THREAD_NUM=$(numactl --hardware | awk '/node 0 cpus:/ {print (NF-3)/2}')
...
cd ..
for dataset in ${tamu_dataset_list[@]}; do
	$NUMACTL --physcpubind=$CORE_RANGE ./bin/heapspgemm $dataset >> $TAMU_1_HEAP_LOG 2>&1
done
bash clean.sh
cd ${CUR_DIR}
...
TAMU_1_HEAP_HYP_LOG=${DIR_PATH}/tamu_1_heap_hyp.log
rm $TAMU_1_HEAP_HYP_LOG
THREAD_NUM=$(numactl --hardware | awk '/node 0 cpus:/ {print (NF-3)/2}')
THREAD_NUM=$((2 * THREAD_NUM))
CORE_RANGE="0-31,64-95"
cd ../..
...
cd ..
for dataset in ${tamu_dataset_list[@]}; do
	$NUMACTL --physcpubind=$CORE_RANGE ./bin/heapspgemm $dataset >> $TAMU_1_HEAP_HYP_LOG 2>&1
done
bash clean.sh
cd ${CUR_DIR}
