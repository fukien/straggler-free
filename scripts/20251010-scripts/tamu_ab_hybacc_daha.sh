...
TAMU_1_AB_HYBACC_DAHA_LOG=${DIR_PATH}/tamu_1_ab_hybacc_daha.log
rm $TAMU_1_AB_HYBACC_DAHA_LOG
cd ../..
bash clean.sh
mkdir build
cd build
...
for dataset in ${tamu_dataset_list[@]}; do
	$NUMACTL --physcpubind=$CORE_RANGE ./bin/ab_hybaccspgemm $dataset >> $TAMU_1_AB_HYBACC_DAHA_LOG 2>&1
done
bash clean.sh
cd ${CUR_DIR}
...
make -j $(( $(nproc) / 16 ))
cd ..
$NUMACTL --physcpubind=$CORE_RANGE ./bin/ab_hybaccspgemm webbase-1M >> $TAMU_1_AB_HYBACC_DAHA_LOG 2>&1
bash clean.sh
cd ${CUR_DIR}
