...
export OMP_SCHEDULE="dynamic,${ompdnr}"
echo "$OMP_SCHEDULE"

ER_6_OMPDNR_LOG=${DIR_PATH}/er_6_ompdnr${ompdnr}.log
rm $ER_6_OMPDNR_LOG
G500_6_OMPDNR_LOG=${DIR_PATH}/g500_6_ompdnr${ompdnr}.log
rm $G500_6_OMPDNR_LOG
SSCA_6_OMPDNR_LOG=${DIR_PATH}/ssca_6_ompdnr${ompdnr}.log
rm $SSCA_6_OMPDNR_LOG
THREAD_NUM=$(numactl --hardware | awk '/node 0 cpus:/ {print (NF-3)/2}')
CORE_RANGE="32-63,96-127"
cd ../..
bash clean.sh
...
for scale in 18 19 20 21; do
	for edge_factor in 1 2 4 8 16 32; do
		...
		$NUMACTL --physcpubind=$CORE_RANGE ./bin/omphashspgemm $dataset >> $ER_6_OMPDNR_LOG 2>&1
	done
done
for scale in 17 18 19 20; do
	for edge_factor in 1 2 4 8 16 32; do
		dataset="g500_${scale}_${edge_factor}"
		$NUMACTL --physcpubind=$CORE_RANGE ./bin/omphashspgemm $dataset >> $G500_6_OMPDNR_LOG 2>&1
	done
done
for scale in 17 18 19 20; do
	for edge_factor in 1 2 4 8 16 32; do
		dataset="ssca_${scale}_${edge_factor}"
		$NUMACTL --physcpubind=$CORE_RANGE ./bin/omphashspgemm $dataset >> $SSCA_6_OMPDNR_LOG 2>&1
	done
done
bash clean.sh
cd ${CUR_DIR}
...
ER_1_OMPDNR_LOG=${DIR_PATH}/er_1_ompdnr${ompdnr}.log
rm $ER_1_OMPDNR_LOG
G500_1_OMPDNR_LOG=${DIR_PATH}/g500_1_ompdnr${ompdnr}.log
rm $G500_1_OMPDNR_LOG
SSCA_1_OMPDNR_LOG=${DIR_PATH}/ssca_1_ompdnr${ompdnr}.log
rm $SSCA_1_OMPDNR_LOG
THREAD_NUM=$(numactl --hardware | awk '/node 0 cpus:/ {print (NF-3)/2}')
...
for scale in 18 19 20 21; do
	for edge_factor in 1 2 4 8 16 32; do
		dataset="er_${scale}_${edge_factor}"
		$NUMACTL --physcpubind=$CORE_RANGE ./bin/omphashspgemm $dataset >> $ER_1_OMPDNR_LOG 2>&1
	done
done
for scale in 17 18 19 20; do
	for edge_factor in 1 2 4 8 16 32; do
		dataset="g500_${scale}_${edge_factor}"
		$NUMACTL --physcpubind=$CORE_RANGE ./bin/omphashspgemm $dataset >> $G500_1_OMPDNR_LOG 2>&1
	done
done
for scale in 17 18 19 20; do
	for edge_factor in 1 2 4 8 16 32; do
		dataset="ssca_${scale}_${edge_factor}"
		$NUMACTL --physcpubind=$CORE_RANGE ./bin/omphashspgemm $dataset >> $SSCA_1_OMPDNR_LOG 2>&1
	done
done
bash clean.sh
cd ${CUR_DIR}
